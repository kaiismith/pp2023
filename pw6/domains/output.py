...
class Output:
    ...
    # Display student infomation on the screen
            ...
            with gzip.open(dat_file, 'rb') as f:
                ...
                if 'file1' in data:
                    with open(file1, 'a') as f1, open(file1, 'r') as f1_read:
                        for line in data['file1'].decode("utf-8"):
                            if re.match(r"^BI\d{2}-\d{3}$", line[0:8]) and line[0:8] not in f1_read.read():
                                f1.write(f"{line}\n")

                # courses.txt saved with format ICT-xxx - Name - Credits
                if 'file2' in data:
                    with open(file2, 'a') as f2, open(file2, 'r') as f2_read:
                        for line in data['file2'].decode("utf-8"):
                            if re.match(r"^ICT-\d{3}$", line[0:7]) and line[0:7] not in f2_read.read():
                                f2.write(f"{line}\n")

                # Others
                if 'file3' in data:
                    with open(file3, 'a') as f3, open(file3, 'r') as f3_read:
                        for line in data['file3'].decode("utf-8"):
                            if line not in f3_read.read() and line[1:7] not in f3_read.read():
                                f3.write(f"{line}\n")
        else:
            return
